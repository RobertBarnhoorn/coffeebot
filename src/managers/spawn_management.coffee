...
desired = (role) ->
  switch role
    when roles.HARVESTER        then 1 * numSources + 1
    when roles.UPGRADER         then 1 * numSources + 1
    when roles.ENGINEER         then 1 * numSources + 1
    when roles.TRANSPORTER      then 1 * numSources + 1
    when roles.RESERVER
      flagCount[flag_intents.RESERVE]
    when roles.CLAIMER
      flagCount[flag_intents.CLAIM]
    when roles.SOLDIER
      flagCount[flag_intents.DEFEND] * 2 or flagCount[flag_intents.ATTACK] * 2 or flagCount[flag_intents.INVADE] * 2
    when roles.SNIPER
      flagCount[flag_intents.DEFEND] * 2 or flagCount[flag_intents.ATTACK] * 2 or flagCount[flag_intents.INVADE] * 2 or flagCount[flag_intents.PATROL] * 1
    when roles.MEDIC
      flagCount[flag_intents.DEFEND] * 2 or flagCount[flag_intents.ATTACK] * 2 or flagCount[flag_intents.INVADE] * 4
  ...
  for role,count of actual
    if count < desired role
      ...
      for u in values units
        if u.memory.role == role and u.ticksToLive < 200 and not u.memory.replaced?
         candidates.push role
